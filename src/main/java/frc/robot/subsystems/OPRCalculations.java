...
import frc.robot.CSVofOPRs;
import frc.robot.OPRProcessing;
...
public class OPRCalculations{
    ...
    private static OPRProcessing.Match matchData = processing.new Match(); //to access inner nested class

    //populates playingTeams[match index][red=0 or blue=1][#teams per alliance] (reads from matchData)
    ...
    public static int[] getPlayingTeams(int teamsPerAlliance) {
        int[][][] playingTeams = new int[processing.get(matches.size())][2][teamsPerAlliance];
    
        for (int i = 0; i < processing.get(matches.size()); i++) { 
            for (int k = 0; k < teamsPerAlliance; k++) {
                //if team is in match = 1 else = 0
                if (matchData.get(i) == redTeamNumbers[0] || matchData.get(i) == redTeamNumbers[1] || matchData.get(i) == redTeamNumbers[2]) {
                    playingTeams[i][0][k] = 1; 
                } else {
                playingTeams[i][0][k] = 0
                }
                if (matchData.get(i) == blueTeamNumbers[0] || matchData.get(i) == blueTeamNumbers[1] || matchData.get(i) == blueTeamNumbers[2]) {
                playingTeams[i][1][k] = 1;
                } else {
                playingTeams[i][1][k] = 0;
                }
            }
        }
    }
    ...
    //populates scores[#match points][red=0 or blue=1]  (reads from matchData based off teamList ordering)
    ...
    public static int getTeamScores(int teamsPerAlliance, int[] teamList) {
        ...
        HashSet<Integer> teamListSet = teamList.iterator();
        while (teamListSet.hasNext()) {
            for (int i = 0; i < processing.matches.size()*3; i++) {
                for (int j = 0; j < 2; j++) {
                   if (j = 0) {
                        scores[i][j] = processing.matches.get(redTeamScore[i]);
                   } else {
                        scores[i][j] = processing.matches.get(blueTeamScore[i]);
                   }
                }
            }
        }
   ...
   //teamList[team number], scores[#match points][red=0 or blue=1], playingTeams[match index][red=0 or blue=1][#teams per alliance]
        ...
        for (int i = 0; i < scores.length; i++) {
            if (scores[i][0] > 0) {
                for (int j = 0; j < teamsPerAlliance; j++) {
                    ...
                    Ar.set(match, Ar.indexOf(playingTeams[i][0][j]), 1); 
                    Ab.set(match, Ab.indexOf(playingTeams[i][1][j]), 1);
                }
            //row, column, score
    ...
    public static double[] computeOPRwithChloesky(double mmse, int[] teamList, int teamsPerAlliance, int[][][] playingTeams, int[][] scores, double averageTeamOffenseScore) {
        ...
        Matrix AoNew = Ao.getTriangular(Ao, 2*numScoredMatches, numTeams);
        ...
        return opr; //and teamList 
    }

    //returns data from computeOPR
    public static double[] data(double mmse, int[] teamList, int teamsPerAlliance, int[][][] playingTeams, int scores, int[] opr) {
        CSVofOPRS csvFile = new CSVofOPRS();
        if (teamList.length < 50) {
            computeOPRwithMMSE(mmse, teamList, teamsPerAlliance, playingTeams, scores);
            return csvFile.getCSVofOPRs(teamList, opr);
        } else if (teamList.length >= 50) {
